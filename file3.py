...
print('第一轮训练结束，当前roc值为：{:.4f}'.format(roc_auc_score(np.array(test_y), np.array(out))))
# 开始更新模型
...
for i in range(num_update):
   ...
   # print('经过{}次更新，当前deFmNu的roc值为:{}'.format(i, roc_val2))
    ...
    print('经过{}次更新，当前ComDeep的roc值为:{:.4f}'.format(i, roc_auc_score(test_y, out)))
    total_roc_es.append(roc_auc_score(test_y, out))
    if end == num_test:
        break

plt.plot(gbdt_roc_es, color='b', lw=2, label='gbdt')
plt.scatter(list(range(len(gbdt_roc_es))), gbdt_roc_es, color='g', marker='^', s=40)
